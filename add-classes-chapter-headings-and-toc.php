function format_post_content($content = '') {
    ...
    $doc = new DOMDocument();
    $doc->registerNodeClass('DOMElement');
    global $wp;
    ...
    /*------------------------------------*\
      Add Class Names to All HTML Elements
    \*------------------------------------*/
    
    /* This part of the function adds class names to every HTML element with
    // the exact element's name.
    //
    // This reduces the risk of malformed statements in CSS that might result
    // in the entire statement getting ignored by browsers that don't recognize
    // a particular element. (E.g., if a browser doesn't recognize the "aside" 
    // element, it won't ignore the CSS statement if listed as ".aside" in the
    // CSS). It's useful in a reset file.
    //
    // Also, it seemed to reduce paint time when used (although I'm not totally
    // sure if or why that would be correct). */
    
    foreach ( array( 'a' ) as $aSingular ) {
        $as = $doc->getElementsByTagName( 'a' );
        foreach ( $as as $asingle ) {
            $oldaClass = $asingle->getAttribute( 'class' );
            $newaClass = "a " . $oldaClass;
	...
	/*------------------------------------------------------*\
		Add "Chapters" and Generate Custom Table of Contents
    \*------------------------------------------------------*/
	
    // This WILL break your page if the content isn't formatted correctly
    
    if ( is_single() && !is_front_page() && !is_attachment() && !is_page() ) {
        // remove <!DOCTYPE 
        ...
        foreach ( array( 'h2' ) as $h ) {
            ...
            foreach ( $h2_headings as $heading ) {
                ...
                $parentDiv->setAttribute( 'class', 'div chapter-title-wrapper chapter-' . $h2_i . '-title-wrapper' );
                ...
                if ($h2_i == 1) {
                    ...
                    $insertedtocUL->setAttribute('class', 'ul toc_list');
                    $insertedtocUL->setAttribute('id', 'table-of-contents-ul');
                    $insertedtocUL->setIdAttribute('inner_toc_list');
                    ...
                    $tocSpan1->setAttribute('class', 'span toc-chapter-number toc-chapter-number-ch-' . $h2_i);
                    $LInodeValueInserted = $insertedtocA->parentNode->firstChild->appendChild( $LInodeValue );
                    $LInodeValue->setAttribute('class', 'span toc-chapter-title toc-chapter-title-ch-' . $h2_i);
                    $insertedtocA->setAttribute('href', $current_url . '/#' . $slug );
                    $insertedtocA->setAttribute( 'data-href-replace', $current_url . '/#' . $anchorSlug );
                    $insertedtocA->setAttribute('class', 'a toc-chapter-link toc-chapter-link-' . $h2_i);
                    ...
                    $newnode->setAttribute( 'class', 'a h2-anchorlink permalink' );
                    $newnode->setAttribute( 'href', $current_url . '/#' . $slug );
                    $newnode->setAttribute( 'data-href-replace', $current_url . '/#' . $anchorSlug );
                    $aText = $doc->createElement('span', ' &#128279;');
                    $aText->setAttribute( 'class', 'span sr-or-no-svg-only' );
                    ...
                    foreach ( $h3_headings as $heading3 ) {
                        ...
                        $parentDiv3->parentNode->setAttribute( 'class', 'section-section section-' . $h2_i . '-' . $h3_i . '-section' );
                        ...
                        $para3 = $doc->createElement( 'p', '§&nbsp;' . $h2_i .'.'. $h3_i );
                        $h3prefix = $heading3->parentNode->insertBefore( $para3, $heading3 );
                        ...
                        if ($h3_i == 1) {
                            $parentDiv3->parentNode->setAttribute( 'class', 'section section-section first-section-of-chapter section-' . $h2_i . '-' . $h3_i . '-section' );
                            ...
                            $insertedtocUL3->setIdAttribute('inner_toc_section_list-'. $h2_i .'-'. $h3_i );
                        };
                        ...
                        $insertedtocLI3->setAttribute('data-section-story', $h3_i);
                        ...
                        $tocSpan3->setAttribute('class', 'span toc-section-number toc-section-number-' . $h2_i . '-' . $h2_i);
                        $LInodeValue3Inserted = $insertedtocA3->parentNode->firstChild->appendChild( $LInodeValue3 );
                        $LInodeValue3->setAttribute('class', 'span toc-section-title toc-section-title-section-' . $h2_i . '-' . $h3_i );
                        $insertedtocA3->setAttribute('href', $current_url . '/#' . $slug3 );
                        $insertedtocA3->setAttribute( 'data-href-replace', $current_url . '/#' . $anchorSlug3 );
                        $insertedtocA3->setAttribute('class', 'a toc-section-link toc-section-link-' . $h2_i . '-' . $h3_i );
                        ...
                        $a3 = $doc->createElement( 'a', '§&nbsp;' . $h2_i .'.'. $h3_i );
                        $h3prefix = $parentDiv3->insertBefore( $para3, $heading3 );
                        $h3prefix->setAttribute( 'class', 'p h3-prefix section-'. $h2_i .'-'. $h3_i );
                        $newnode3 = $para3->appendChild( $a3 );
                        $newnode3->setAttribute( 'class', 'a h3-anchorlink permalink' );
                        $heading3->setAttribute( 'id', $slug3 );
                        $newnode3->setAttribute( 'href', $current_url . '/#' . $slug3 );
                        $newnode3->setAttribute( 'data-href-replace', $current_url . '/#' . $anchorSlug3 );
                        ...
                        foreach ( $h4_headings as $heading4 ) {
                            ...
                            $parentDiv4->parentNode->setAttribute( 'class', 'section subsection-section first-subsection-of-chapter-section subsection-'. $h2_i .'-'. $h3_i .'-'. $h4_i .'-section' );
                            ...
                            if ($h4_i == 1) {
                                ...
                                $insertedtocUL4->setIdAttribute('inner_toc_subsection_list-'. $h2_i .'-'. $h3_i .'-'. $h4_i);
                            } elseif ($h4_i == $lastH4) {
                                $parentDiv4->parentNode->setAttribute( 'class', 'section subsection-section last-subsection-of-chapter-section subsection-'. $h2_i .'-'. $h3_i .'-'. $h4_i .'-section' );
                            };
                            ...
                            $tocSpan4->setAttribute('class', 'span toc-subsection-number toc-subsection-number-' . $h2_i .'-'. $h3_i .'-'. $h4_i );
                            $LInodeValue4Inserted = $insertedtocA4->parentNode->firstChild->appendChild( $LInodeValue4 );
                            $LInodeValue4->setAttribute('class', 'span toc-subsection-title toc-subsection-title-section-' . $h2_i . '-' . $h3_i .'-'. $h4_i );
                            $insertedtocA4->setAttribute('href', $current_url . '/#' . $slug4 );
                            $insertedtocA4->setAttribute( 'data-href-replace', $current_url . '/#' . $anchorSlug4 );
                            $insertedtocA4->setAttribute('class', 'a toc-subsection-link toc-subsection-link-' . $h2_i . '-' . $h3_i .'-'. $h4_i );
                            ...
                            $newnode4->setAttribute( 'class', 'a h4-anchorlink permalink' );
                            $heading4->setAttribute( 'id', $slug4 );
                            $newnode4->setAttribute( 'href', $current_url . '/#' . $slug4 );
                            $newnode4->setAttribute( 'data-href-replace', $current_url . '/#' . $anchorSlug4 );
                            $h4_i+=1;
                            }
                        if ($h3_i == $lastH3) {
                            $heading->parentNode->parentNode->setAttribute( 'class', 'section last-section section-section section-' . $h2_i . '-' . $h3_i . '-section' );
                        };
                        $h3_i+=1;
                    }
                } else {
                    ...
                    foreach ( $h3_headings as $heading3 ) {
                        ...
                        $heading3->setAttribute( 'aria-level', '3' );
                        $heading3->setAttribute( 'tabindex', '0' );
                        $heading3->setAttribute( 'itemprop', 'subheading');
                        $heading3->setAttribute( 'class', 'h3 section-heading section-' . $h2_i . '-' . $h3_i . '-heading' );
                        $parentDiv3->parentNode->setAttribute( 'class', 'section section-section section-' . $h2_i . '-' . $h3_i . '-section' );
                        $slug3 = $tmpslug3 = sanitize_title( $heading3->nodeValue );
                        $i3 = 2;
                        while ( false !== in_array( $slug3, $anchors3 ) ) {
                            $slug3 = sprintf( '%s-%d', $tmpslug3, $i3++ );
                        ...
                        if ($h3_i == 1) {
                            ...
                            $parentDiv3->parentNode->setAttribute( 'class', 'section section-section last-section-of-chapter section-' . $h2_i . '-' . $h3_i . '-section' );
                        };
                        $tocSpan3 = $doc->createElement( 'span', '§&nbsp;' . $h2_i .'.'. $h3_i );
                        $LInodeValue3 = $doc->createElement( 'span', $heading3->nodeValue );
                        $tocLI3 = $doc->createElement( 'li' );
                        ...
                        $insertedtocA3 = $insertedtocLI3->appendChild( $tocA3 );
                        $tocSpan3Inserted = $insertedtocA3->parentNode->firstChild->appendChild( $tocSpan3 );
                        $tocSpan3->setAttribute('class', 'span toc-section-number toc-section-number-' . $h2_i .'-'. $h3_i);
                        $LInodeValue3Inserted = $insertedtocA3->parentNode->firstChild->appendChild( $LInodeValue3 );
                        $LInodeValue3->setAttribute('class', 'span toc-section-title toc-section-title-section-' . $h2_i . '-' . $h3_i );
                        $insertedtocA3->setAttribute('href', $current_url . '/#' . $slug3 );
                        $insertedtocA3->setAttribute( 'data-href-replace', $current_url . '/#' . $anchorSlug3 );
                        ...
                        $a3 = $doc->createElement( 'a', '§&nbsp;' . $h2_i .'.'. $h3_i );
                        $h3prefix = $parentDiv3->insertBefore( $para3, $heading3 );
                        $h3prefix->setAttribute( 'class', 'p h3-prefix sub-section-'. $h2_i .'-'. $h3_i );
                        $newnode3 = $para3->appendChild( $a3 );
                        $newnode3->setAttribute( 'class', 'a h3-anchorlink ' );
                        $heading3->setAttribute( 'id', $slug3 );
                        $newnode3->setAttribute( 'href', $current_url . '/#' . $slug3 );
                        $newnode3->setAttribute( 'data-href-replace', $current_url . '/#' . $anchorSlug3 );
                        $h4_headings = $heading3->parentNode->parentNode->getElementsByTagName( 'h4' );
                        ...
                        foreach ( $h4_headings as $heading4 ) {
                            ...
                            $parentDiv4->appendChild($heading4);
                            $heading4->setAttribute( 'aria-level', '4' );
                            $heading4->setAttribute( 'itemprop', 'subheading');
                            $heading4->setAttribute( 'tabindex', '0' );
                            $heading4->setAttribute( 'class', 'h4 subsection-heading subsection-' . $h2_i . '-' . $h3_i . '-' . $h4_i . '-heading' );
                            $parentDiv4->parentNode->setAttribute( 'class', 'section subsection-section subsection-' . $h2_i . '-' . $h3_i . '-' . $h4_i . '-section' );
                            $slug4 = $tmpslug4 = sanitize_title( $heading4->nodeValue );
                            $i4 = 2;
                            while ( false !== in_array( $slug4, $anchors4 ) ) {
                                $slug4 = sprintf( '%s-%d', $tmpslug4, $i4++ );
            ...
            $h3_ListItems = $doc->getElementsByTagName( 'li' );
            $h30_i = 0;
            foreach ( $h3_ListItems as $h3_ListItem ) {
                if ($h3_ListItem->hasAttribute( 'data-section-story' )) {
                    $h3_ListItem->setAttribute( 'data-section-story', $h30_i );
                    $h30_i+=1;
                }
...
add_filter( 'the_content', 'format_post_content', 9, 1 );
